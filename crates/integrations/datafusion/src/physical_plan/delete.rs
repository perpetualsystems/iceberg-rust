...
/// classify files, commit a [`ReplaceDataFilesAction`](iceberg::transaction::ReplaceDataFilesAction)
/// (used in pure-delete mode — no added files) that drops the fully-covered files,
/// and emit a single `count` row.
#[derive(Debug)]
...
async fn run_partition_aligned_delete(
    ...
    let mut filters_by_spec: std::collections::HashMap<i32, PartitionCoverageFilter> =
            ...
            let spec_filter = match filters_by_spec.get(&spec_id) {
                ...
                None => {
                    let partition_spec = table
                        ...
                        .clone();
                    let filter =
                        build_coverage_filter(predicate.as_ref(), &schema, partition_spec)?;
    ...
    let action = tx.replace_data_files().delete_files(to_delete);
    action.apply(tx)?.commit(catalog).await?;

    Ok(total_record_count)
}

fn build_coverage_filter(
    predicate: Option<&Predicate>,
    schema: &iceberg::spec::SchemaRef,
    partition_spec: PartitionSpecRef,
) -> iceberg::Result<PartitionCoverageFilter> {
    // Empty filter means "every file is fully covered" regardless of spec.
    let pred = predicate.unwrap_or(&Predicate::AlwaysTrue);
    PartitionCoverageFilter::try_new(pred, schema.clone(), partition_spec, true)
}
